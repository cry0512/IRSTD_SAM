...
# Build model (no checkpoint) and attach gates
model = build_efficient_sam(encoder_patch_embed_dim=192, encoder_num_heads=3, checkpoint=None)
model.eval()

# Attach both gates with small defaults
try:
    dim = model.image_encoder.neck[0].out_channels
except Exception:
    dim = 256
model.image_encoder.freq_gate = FreqGate(dim, patch_size=8)
...
# 2) Predict masks with 1 query and a couple of points
max_num_queries = 1
num_pts = 2
